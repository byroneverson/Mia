...
class TokenizerGPT2 {
    ...
    init(_ modelURL: URL) {
        // Merges - preprocesses text from human readable into vocab partial words
        let mergesURL = modelURL.appendingPathComponent("merges").appendingPathExtension("txt")
        guard let bpeMergesTxt = try? String(contentsOf: mergesURL) else {
            print("TokenizerGPT2 init error merges.txt")
            return
        }
        ...
        // Vocab - Convert vocabulary chunk to numerical index
        let vocabURL = modelURL.appendingPathComponent("vocab").appendingPathExtension("json")
        guard let vocabData = try? Data(contentsOf: vocabURL) else {
            print("TokenizerGPT2 init error vocab.json")
            return
        }
        let jsonDecoder = JSONDecoder()
        guard let vocabDict = try? jsonDecoder.decode([String: Int].self, from: vocabData) else {
            print("TokenizerGPT2 init error encoder nil")
            return
        }
    ...
    func byteEncode(text: String) -> [String] {
        ...
        let tokens = text.ranges(of: regex).map { String(text[$0]) }
    ...
    func tokenize(text: String) -> [String] {
        ...
        for token in self.byteEncode(text: text) {
            let nextToken = self.bpe(token: token).split(separator: " ").map { String($0) }
            tokens.append(contentsOf: nextToken)
            print("🧙 \(nextToken)")
        }
        return tokens
    }
