...
int main(int argc, char ** argv) {
    ggml_common_params params;
    params.model = "models/stablelm/ggml-stablelm-7b-sft-v7-epoch-3-q4_0.bin";

    if (ggml_common_params_parse(argc, argv, params) == false) {
        return 1;
    }
    ...
    if (params.random_prompt) {
        params.prompt = ggml_common_random_prompt(rng);
    }
...
//bool is_prime(int n) {)";
    ...
    // uncomment the "used_mem" line in gptneox.cpp to see the results
    if (params.mem_test) {
        {
    ...
    // tokenize the prompt
    auto embd_inp = ::gptneox_tokenize(ctx, params.prompt); //, true);
    ...
    const auto inp_pfx = ::gptneox_tokenize(ctx, "<|prompter|>"); //, true);
...
#endif
               " - Press Return to return control to model.\n"
    ...
    while (n_remain != 0 || params.interactive) {
        ...
        if (embd.size() > 0) {
            ...
            if (gptneox_eval(ctx, embd.data(), embd.size(), n_past, params.n_threads)) {
        ...
        // end of text token
        if (embd.back() == gptneox_token_eos()) {
            if (params.instruct) {
                is_interacting = true;
            } else {
